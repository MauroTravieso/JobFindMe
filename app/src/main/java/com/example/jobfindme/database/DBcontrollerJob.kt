...
class DBcontrollerJob (c: Context) {
    ...
    fun getAllJobs(): Cursor {
        val allColumns= arrayOf<String>(DBJob.JOB_COLUMN_TITLE,DBJob.JOB_COLUMN_COMPANY,DBJob.JOB_COLUMN_status,DBJob.JOB_COLUMN_VIEW)
    ...
    fun updateJobs(job: Job):Int {
        ...
        val i = database.update(DBJob.JOB_TABLE, cvUpdate,
            DBJob.JOB_COLUMN_TITLE + " = " + job.title, null)
        return i
    }
    // Deleting record data from table by NAME
    fun deleteJob(title:String) {
        database.delete(DBJob.JOB_TABLE, (DBJob.JOB_COLUMN_TITLE + "="
                + title), null)
    }
}